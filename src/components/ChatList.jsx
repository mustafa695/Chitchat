...
const ChatList = () => {
  ...
  const fetchUsersChatList = async () => {
    try {
      ...
      const chatUser = response.data?.filter((x) => x?._id !== user)[0];
  ...
  const fetchChannels = async () => {
    try {
      const response = await getChannels(user);
  ...
  return (
    <div
      className="min-w-80 max-w-80 h-svh bg-white"
      style={{
              ...
              <div className="relative">
                ...
                <div
                  className={`size-2 border-2 border-white rounded-full absolute bottom-0 -right-0.5 ${
                    onlineUsers[item?._id] ? "bg-[#06d6a0]" : "bg-gray-300"
                  }`}
                ></div>
              </div>
              <p
